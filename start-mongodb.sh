...
wait_for_mongodb () {
  ...
  until docker exec --tty $MONGODB_NAME $MONGODB_CLIENT --port $MONGODB_PORT $MONGODB_ARGS --eval "db.serverStatus()"
  do
    echo "."
    sleep 1
    TIMER=$((TIMER + 1))
...
if [ -z "$MONGODB_REPLICA_SET" ]; then
  ...
  docker run --name $MONGODB_NAME --publish $MONGODB_PORT:$MONGODB_PORT -e MONGO_INITDB_DATABASE=$MONGODB_DB -e MONGO_INITDB_ROOT_USERNAME=$MONGODB_USERNAME -e MONGO_INITDB_ROOT_PASSWORD=$MONGODB_PASSWORD --detach mongo:$MONGODB_VERSION --port $MONGODB_PORT
...
docker run --name $MONGODB_NAME --publish $MONGODB_PORT:$MONGODB_PORT --detach mongo:$MONGODB_VERSION --replSet $MONGODB_REPLICA_SET --port $MONGODB_PORT
...
docker exec --tty $MONGODB_NAME $MONGODB_CLIENT --port $MONGODB_PORT --eval "
  rs.initiate({
...
docker exec --tty $MONGODB_NAME $MONGODB_CLIENT --port $MONGODB_PORT --eval "rs.status()"
echo "::endgroup::"
